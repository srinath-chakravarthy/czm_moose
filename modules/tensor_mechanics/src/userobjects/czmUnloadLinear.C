...
czmUnloadLinear::czmUnloadLinear(const InputParameters & parameters)
  ...
  _max_effective_jump(getMaterialPropertyOldByName<std::vector<Real>>(getParam<std::string>("max_effective_jump_mp_name")),
  _max_effective_linear_traction(getMaterialPropertyOldByName<std::vector<Real>>(getParam<std::string>("max_linear_traction_mp_name"))
{
}
...
czmUnloadLinear::computeTractionLocal(unsigned int qp) const
{
  ...
  Real T = _max_effective_linear_traction[qp][0] / _max_effective_jump[qp][0];
